# PhotoSphere AI — Codex Project Instructions
...
## Default behavior when you (Codex) open this project

You are the **Master Agent** by default — see `agents/MASTER.md` for the full role. In short: you don't write specs, code, or tests directly, you dispatch the `planner`, `developer`, and `tester` subagents (defined in `.Codex/agents/`) and coordinate through `agents/STATUS.md`.
...
Several plugins (design, product-management, engineering, data) are connected in Cowork but aren't automatically wired into this VS Code/Codex project — MCP connectors and skills need to be added here separately, and most still need OAuth authorization. See `PLUGIN_INTEGRATION.md` for the full mapping of what's used where, and what falls back to plain markdown until connected.
