...
| `design:figma` (MCP) | **Developer** | Stage 2 of the UI flow: turn the SVG Abhishek picked into a real Figma frame. **Caveat:** many Figma MCP integrations are read-only (they read/inspect existing files, generate code from designs) and can't create new files. Verify this once authorized — if it can't write, the chosen SVG stays the design record instead. | Needs auth + verify write capability |
...
Pick 2–3 of the "needs plugin added to Claude Code" rows above — Figma (once you confirm write access) and the Planner skills (write-spec/sprint-planning) are the highest-leverage ones given what you asked for. Authorize + add those to Claude Code first, run one `/orchestrate` cycle, and see how much it actually changes before wiring in the rest.
